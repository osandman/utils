...
//        System.out.println(getAverageSumFromSingleDigits("Алгебра\tП 5 3 4 П 4 П 4 3 П Б Б Б 2 П 4 3 П П 4", 2));
        String all = "1\tАлгебра\tП 5 3 4 П 4 П 4 3 П Б Б Б 2 П 4 3 П П 4\t0\t10\t3\t\n" +
                     "2\tАнгл. яз.\t3 П 3 4 П 4 3 4 П П П Б 4 П 2 П П \t0\t9\t1\t\n" +
                     "3\tБилет в будущее\tП Б П П \t0\t4\t1\t\n" +
                     "4\tБиология\tП 4 2 5 4 Б П П \t0\t4\t1\t4\n" +
                     "5\tГеография\tП 3 3 4 П Б 4 П 3 4 П \t0\t5\t1\t4\n" +
                     "6\tГеометрия\t3 4 П 3 П П Б 4 4\t0\t4\t1\t\n" +
                     ...
                     "14\tОбществознание\t3 4 П 4 3\t0\t1\t0\t\n" +
                     "15\tРазговоры о важном\tП Б \t0\t2\t1\t\n" +
                     "16\tРодная литература\tП Б 5 П 5\t0\t3\t1\t\n" +
                     ...
                     "21\tФизика\t2 4 П 5 П 3 5 П П Б 2 П 4 П П 2\t0\t8\t1\t\n" +
    ...
    public static String getAllAverageGrades(String input) {
        ...
        for (String line : lines) {
            ...
            builder.append(temp[0]).append(". ").append(temp[1]).append(" - средний балл: ")
                    .append(getAverageSumFromSingleDigits(temp[2], PLACES))
                    .append("\n");
        }
    ...
    public static double getAverageSumFromSingleDigits(String input, int places) {
        ...
        try {
            Double.parseDouble(onlyDigits[0]);
